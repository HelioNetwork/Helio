...
bool CHelioStake::InitFromTxIn(const CTxIn& txin)
{
...
bool CHelioStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CHelioStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CHelioStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CHelioStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CHelioStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CHelioStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
CDataStream CHelioStake::GetUniqueness() const
{
    //The unique identifier for a HLO stake is the outpoint
...
CBlockIndex* CHelioStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CHelioStake::ContextCheck(int nHeight, uint32_t nTime)
{
