...
#if defined(NDEBUG)
#error "Helio_Coin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uhelio) are considered zero fee (for relaying, mining and transaction creation)
...
bool RecalculateHLOSupply(int nHeightStart)
{
	...
	uiInterface.ShowProgress(_("Recalculating HLO supply..."), 0);
	while (true) {
		if (pindex->nHeight % 1000 == 0) {
			LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
			int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainHeight - nHeightStart))));
			uiInterface.ShowProgress(_("Recalculating HLO supply..."), percent);
		}
...
int64_t GetBlockValue(int nHeight)
{
	...
	else if (nHeight <= 10000 && isPoSActive) {
		nSubsidy = 0.5 * COIN;
	}
	...
	else if (nHeight <= 4139020 && nHeight > 400000) {
		nSubsidy = 12.5 * COIN;
	}
	else {
		nSubsidy = 0 * COIN;
...
bool IsSkippingRange(int nHeight) {
	if ((nHeight > 9999 && nHeight < 10120) || (nHeight > 19999 && nHeight < 20120)
		|| (nHeight > 29999 && nHeight < 30120) || (nHeight > 39999 && nHeight < 40120) || (nHeight > 49999 && nHeight < 50120)
		|| (nHeight > 59999 && nHeight < 60120) || (nHeight > 69999 && nHeight < 70120) || (nHeight > 79999 && nHeight < 80120)
		|| (nHeight > 89999 && nHeight < 90120) || (nHeight > 99999 && nHeight < 100120) || (nHeight > 119999 && nHeight < 120120)
		|| (nHeight > 139999 && nHeight < 140120) || (nHeight > 159999 && nHeight < 160120) || (nHeight > 179999 && nHeight < 180120)
		|| (nHeight > 199999 && nHeight < 200120) || (nHeight > 224999 && nHeight < 225120) || (nHeight > 249999 && nHeight < 250120)
		|| (nHeight > 299999 && nHeight < 300120) || (nHeight > 349999 && nHeight < 350120) || (nHeight > 399999 && nHeight < 400120))
	{
		return true;
	}
	return false;
...
{
    util::ThreadRename("helio-scriptch");
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> helioInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
                helioInputs.push_back(stakeIn);
        }
        const bool hasHLOInputs = !helioInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasHLOInputs) {
                    ...
                    for (const CTxIn& helioIn : helioInputs)
                        if(helioIn.prevout == in.prevout)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasHLOInputs) {
                            for (const CTxIn& stakeIn : helioInputs)
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // Helio_Coin: We use certain sporks during IBD, so check to see if they are
