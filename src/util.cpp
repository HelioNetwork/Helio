...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\Helio_Coin
// Windows >= Vista: C:\Users\Username\AppData\Roaming\Helio_Coin
// Mac: ~/Library/Application Support/Helio_Coin
// Unix: ~/.helio
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Helio";
#else
    fs::path pathRet;
...
{
    fs::path pathConfigFile(GetArg("-conf", "helio.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty helio.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override helio.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(GetArg("-pid", "heliod.pid"));
