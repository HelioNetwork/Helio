...
/**
 * Fees smaller than this (in uhelio) are considered zero fee (for transaction creation)
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(CRecipient{scriptChange, BUDGET_FEE_TX_OLD, false}); // Old 50 HLO collateral
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    std::string strFail = "";
    std::vector<CRecipient> vecSend;
    vecSend.push_back(CRecipient{scriptChange, BUDGET_FEE_TX, false}); // New 5 HLO collateral
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true) {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-helio-address
        ...
        )
{
    ...
    // Parse utxos into CHelioStakes
    std::list<std::unique_ptr<CStakeInput> > listInputs;
    for (const COutput &out : vCoins) {
        std::unique_ptr<CHelioStake> input(new CHelioStake());
