...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::HLO);
    ...
    if (settings.contains("nAnonymizeHelioAmount") || reset)
        SoftSetArg("-anonymizehelioamount", settings.value("nAnonymizeHelioAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in helio.cpp during shotdown to also remove the window positions
