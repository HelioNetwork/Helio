...
#include "qt/helio/forms/ui_addresseswidget.h"
#include "qt/helio/addresslabelrow.h"
#include "qt/helio/addnewaddressdialog.h"
#include "qt/helio/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(Helio_CoinGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        CTxDestination helioAdd = DecodeDestination(address.toUtf8().constData(), isStakingAddress);
        ...
        if (walletModel->updateAddressBookLabels(helioAdd, label.toUtf8().constData(),
