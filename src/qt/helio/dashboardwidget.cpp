...
#include "qt/helio/dashboardwidget.h"
#include "qt/helio/forms/ui_dashboardwidget.h"
#include "qt/helio/sendconfirmdialog.h"
#include "qt/helio/txrow.h"
...
DashboardWidget::DashboardWidget(Helio_CoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareHelio, "square-chart-hlo");
    setCssProperty(ui->labelHelio, "text-chart-hlo");
    ...
    setCssProperty(ui->labelAmountHlo, "text-stake-hlo-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &Helio_CoinGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &Helio_CoinGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &Helio_CoinGUI::openFAQ);
    ...
    // update the display unit, to not use the default ("HLO")
    updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair HLO
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            hlo = (pair.first != 0) ? pair.first / 100000000 : 0;
            chartData->totalHlo += pair.first;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesHlo.append(hlo);

        int max = std::max(hlo, hlo);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(32, 110, 65));
    ...
    set0->append(chartData->valuesHlo);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalHlo > 0) {
        setCssProperty(ui->labelAmountHlo, "text-stake-hlo");
    } else {
        setCssProperty(ui->labelAmountHlo, "text-stake-hlo-disable");
    }
    forceUpdateStyle({ui->labelAmountHlo});
    ui->labelAmountHlo->setText(GUIUtil::formatBalance(chartData->totalHlo, nDisplayUnit));
