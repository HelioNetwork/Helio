...
#include "qt/helio/forms/ui_send.h"
#include "qt/helio/addnewcontactdialog.h"
#include "qt/helio/qtutils.h"
#include "qt/helio/sendchangeaddressdialog.h"
#include "qt/helio/optionbutton.h"
#include "qt/helio/sendconfirmdialog.h"
#include "qt/helio/myaddressrow.h"
#include "qt/helio/guitransactionsutils.h"
...
SendWidget::SendWidget(Helio_CoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-hlo");
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        CTxDestination helioAdd = DecodeDestination(address.toStdString());
        if (walletModel->isMine(helioAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(helioAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
