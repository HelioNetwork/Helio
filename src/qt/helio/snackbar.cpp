...
#include "qt/helio/forms/ui_snackbar.h"
#include "qt/helio/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(Helio_CoinGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &Helio_CoinGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
