Helio Coin version 3.0.2 is now available from:
...
Helio Coin v3.0.2 is a recommended, semi-mandatory update for all users. This release contains transaction creation bug fixes for zHLO spends, automint calculation adjustments, and other various updates/fixes.

zHLO spending requires this update.
...
Auto Wallet Backup
---------------------
In addition to the automatic wallet backup that is done at each start of the client, a new automatic backup function has been added that will, by default, create a backup of the wallet file during each zHLO mint operation (zHLO spends which re-mint their change are also included in this). This functionality is controlled by the `-backupzhelio` command-line option, which defaults to `1` (enabled, auto-backup).

Users that wish to prevent this behavior (not recommended) can pass `-backupzhelio=0` at the command-line when starting the client, or add `backupzhelio=0` to their `helio.conf` file.

zHLO Automint Calculations
---------------------
A bug in the automint calculations was made apparent on mainnet when block times exceeded expectations, resulting in zHLO mint transactions that were in an unconfirmed state to still be treated as if they had never been minted. This caused automint to effectively mint more than what was intended.

RPC Changes
---------------------
The `bip38decrypt` command has had it's parameter order changed to be more consistent with it's counterpart. The command now expects the Helio_Coin address as it's first parameter and the passphrase as it's second parameter.
...
- #292 `feadab4` Additional checks for double spending of zHlo serials. (presstab)

### Wallet
- #271 `5e9a086` [Wallet] Remove unused member wallet in UnlockContext inner class (Jon Spock)
- #279 `e734010` Add -backupzhelio startup flag. (presstab)
- #280 `fdc182d` [Wallet] Fix zHlo spending errors. (presstab)
- #282 `310f216` [Wallet] Count pending zHlo balance for automint. (presstab)
...
- #291 `cb314e6` [Qt] zHlo control quantity/amount fixes (rejectedpromise)
