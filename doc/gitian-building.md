...
*Setup instructions for a gitian build of Helio Coin using a VM or physical system.*

Gitian is the deterministic build process that is used to build the Helio_Coin
...
to the Helio_Coin GitHub release page.
...
- [Building Helio Coin](#building-helio-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/helio-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/helio-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/helio-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/helio-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/helio-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/helio-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/helio-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@helio.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/helio-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/helio-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/helio-osx-*-build.assert
```
